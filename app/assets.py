...
def build_binance_universe(
    ...
    session.headers.update({
        "User-Agent": "V4.2-Binance-Universe-Builder/1.0",
        "Accept": "application/json",
    })

    exchange_url = f"{BINANCE_UNIVERSE_URL}/api/v3/exchangeInfo"
    ticker_url = f"{BINANCE_UNIVERSE_URL}/api/v3/ticker/24hr"
    ...
    print(
        f"[Binance Universe] connus={discovered} | "
        f"TRADING={trading} | candidats Spot/liquides={len(candidates)}"
    )
    ...
    # Volume reste le facteur dominant ; les autres critères départagent
    # les marchés de volume comparable.
    ranked.sort(
        key=lambda x: (
            x["quote_volume_24h"],
            x["liquidity"],
    ...
    # Mapping dynamique : Binance uniquement pour les nouvelles paires.
    ...
    print(
        f"[Binance Universe] sélection finale={len(selected_symbols)} "
        f"sur {len(ranked)} marchés admissibles"
    )
