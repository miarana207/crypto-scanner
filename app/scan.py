...
def scan(symbols, fetcher=klines, interval="5m", limit=1000, threshold=60, pause=0.3):
    ...
    for sym in symbols:
        try:
            ...
            required = ["ema20", "ema50", "rsi", "prev_high", "prev_low"]
            # NB: "relvol" est volontairement exclu de cette liste. Pour les
            # instruments sans vraie donnée de volume (forex, or via Twelve
            # Data — voir data_sources.py), relvol vaut structurellement NaN
            # (0/0). score() gère ça proprement (NaN >= 1.5 est toujours
            # False, donc pas de bonus volume, pas de crash) — mais si on
            # l'exigeait ici, ces catégories seraient exclues en permanence.
